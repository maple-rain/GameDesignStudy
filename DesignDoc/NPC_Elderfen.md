...
제출 전 반드시 고쳐야 하는 항목이다.

| 위치 | 문제 | 원인 |
|---|---|---|
| 로빈 (S17) | 외형 요소 라벨에 **`수녀모`**가 포함됨 | 그레이스(수녀) 슬라이드에서 복사 후 미수정 |
| 벨라 (S19) | 성격이 말콤과 **완전히 동일** (`장난꾸러기 / 말괄량이 / 낙천적 / 고집있는`) | 복사 후 미수정. 24세 주부와 12세 아이가 같은 성격 |
| 마을 이름 | 표지는 **엘더펜**, 마을 컨셉 슬라이드는 **엘데펜** | 표기 불일치 |
| 존 (S12) | 설정 마지막 문장이 잘림 — `"적은 경비병의 수에 플레이어에게"` | 작성 중단 |

> 복사 흔적은 문서 전체의 신뢰를 떨어뜨린다.
> 특히 수녀모와 성격 중복은 "붙여넣고 안 고쳤다"로 읽히므로 우선 수정 대상이다.
