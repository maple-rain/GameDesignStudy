...
> **작업 기간**: 2025-06-11 ~ 2026-08-10 (Ver0.1 → Ver1.03, 10개 버전)
> **산출물**: NPC 14명 (기능 8 / 장식 6)
> **원본**: `박준서_20251120NPC컨셉_Ver1.01.pptx`
> **최신**: `박준서_20260810NPC컨셉_Ver1.03.pptx` — QA 수정 반영 (5장 참고)
> **PDF**: [NPC_Elderfen_Ver1.03.pdf](./pdf/NPC_Elderfen_Ver1.03.pdf) — 15쪽, NPC 1인당 1쪽 + 캐릭터 아트
...
**말콤의 동경은 단독으로 존재하지 않는다.**
`말콤 → 해리스`가 `해리스 → 존`을 경유한다.
...
분류 기준은 외형이나 비중이 아니라 **시스템 기능의 유무**다.
...
납품본을 다시 검수하며 나온 맞춤법과 편집 흔적이다.

| 위치 | 문제 | 수정 |
|---|---|---|
| 마을 컨셉 (S4) | `변경 부위에 자리잡은` — 부위는 신체 부분을 뜻한다 | `변경 부근에` |
| 마을 컨셉 (S4) | `들리기에도 애매한` — 들리다(소리)와 들르다(경유) 혼동 | `들르기에도` |
| 플로라 (S10) | `까질함` | `까칠함` |
| 플로라 (S10) | 외형 라벨에 **`모자를 망토로 변경`** — 아트 수정 지시문이 그대로 남아 있었다 | `검은 망토` |
| 벨라 (S19) | 본문은 `벨벳 튜닉 착용`인데 라벨만 `벨벳 드레스` | `벨벳 튜닉` |

플로라 건이 Ver1.02의 `수녀모`와 같은 계열이다.
**작업 중 메모가 최종본에 남는 실수가 두 번 반복됐다.**
외형 라벨은 아트 발주로 그대로 넘어가는 칸이므로, 다음 버전부터는
납품 전 라벨 칸만 따로 훑는 절차를 두는 것이 맞다.

### 확인이 필요한 항목

| 위치 | 내용 |
|---|---|
| 로빈 (S17) | 체형이 `실압근이 꽉 찬 체형` — 사전에 없는 말이다. `잔근육`을 의도한 것인지 확인 필요. **본인 창작이라 임의로 고치지 않았다** |
| 말콤 (S15) | 성격의 `말괄량이`는 통상 여자아이에게 쓰는 말이다. 12세 남자아이인 말콤에게 의도한 것인지 확인 필요 |
