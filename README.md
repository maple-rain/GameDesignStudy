...
| **분석했다** | 게임 33개 · 설계 개념 6개 · 서비스와 경제 6개 |
...
| [GameAnalysis/](./GameAnalysis) | **메커닉** | 게임 하나를 10단계 템플릿으로 분석 — 33개 |
...
| [TIL/](./TIL) | 날짜별 학습 기록 35개 + [개념 목록과 판단이 바뀐 기록](./TIL/LearnedConcepts.md) |
...
| [던전앤파이터 중천 파밍 구조](./SystemAnalysis/DNF_JungCheon_Farming.md) | 2026-08 | 직접 플레이한 시즌을 뒤늦게 분해했다. **계시 유입이 정확히 1캐릭터분에 맞춰져 있어 그 이상은 전부 재고에서 나온다.** 소울 시세가 등급과 무관하게 계시 1개당 11만 골드로 수렴 — 드랍 운이 다음 파밍 연료를 567배 가른다. **이탈은 재미가 사라진 시점이 아니라 선택지가 하나로 줄어든 시점에 일어났다** · **[PDF](./SystemAnalysis/pdf/DNF_JungCheon_Farming.pdf)** 10쪽 |
| [던전앤파이터 소울 시세는 왜 100분의 1이 되었나](./SystemAnalysis/DNF_Soul_Market.md) | 2026-08 | 같은 시장을 1년 7개월 뒤 다시 열었다. **공개 API로 체결가와 거래 속도를 직접 수집**([도구](./SystemAnalysis/tools)). 거래는 여전히 활발한데 값만 100분의 1 — 수요 소멸이 아니라 **재화의 위치가 바뀐 것**. 등급별 정렬이 깨진 원인이 조율 수요임을 확인. **파밍 비용이 일일 수입의 9.5배에서 9.4%로 내려왔다** · **[PDF](./SystemAnalysis/pdf/DNF_Soul_Market.pdf)** 14쪽 |
| [던전앤파이터 아바타 패키지가 파는 것은 아바타가 아니다](./SystemAnalysis/DNF_TropicalPackage_BM.md) | 2026-08 | 판매 중인 상품을 종료 전에 분해했다. **구성품 값의 59.1%가 오라 한 칸**에 실려 있고 그 칸만 확률형이다. 확률 기대값과 상자 시세의 오차 **1.9%** — 시장이 스스로 계산해 놓았다. 되팔아 남는 값은 없다(수수료 반영 **0.09%**). **같은 물건인데 기준을 바꾸면 가장 싼 경로가 뒤집힌다.** 108종을 2시간 간격으로 수집([도구](./SystemAnalysis/tools)) · 확률 공시 전량 대조 · 삭제 규칙까지 추적. 의도 8개와 개선점 6개, 화폐 통합 제안까지 · **[PDF](./SystemAnalysis/pdf/DNF_TropicalPackage_BM.pdf)** 43쪽 |
...
| [보스 무브셋 — 의도한 대로 흘러가지 않은 지점들](./DesignDoc/Boss_Moveset.md) | 2026-09 | 팀 프로젝트 Enemy 파트(보스 1체, 기술 25종)를 9일간 만들면서 **내가 정한 것이 실행에서 어떻게 어긋났는지** 기록. 애니메이션을 붙인 순간 **이름이 세 겹**(원본 파일명 / 애니메이터 상태명 / 기술명)이 되고 코드는 정수 하나로 연결된다 — 세 겹이 어긋나도 게임은 정상 작동하고, 결국 **내 프로젝트에서 내가 넣은 애니메이션인지 확인할 수 없게 됐다.** 그래서 이름을 어떻게 정리해야 하는지 규칙 4개로 정리. 그 외 비율의 분모, 곱셈으로 쓴 규칙이 나중에 준 자유도, 고정값이 죽인 다양성(도약기 30% → 18.8%), 조용히 실패하는 의존성. **분석하던 눈과 설계하는 눈 10항목 대조표** 포함 |
...
| [Monster Hunter: World](./GameAnalysis/MonsterHunterWorld.md) | 안전하게 공격할 수 있는 기회 | 파밍 루프 위에 겹친 학습 루프 · 반복이 노동이 되는 지점 |
| [Sleeping Dogs](./GameAnalysis/SleepingDogs.md) | 전투에 쓸 수 있는 주변 환경 | 오픈월드의 존재 이유 · 요소를 판정하는 5개 층위 |
| [Dishonored](./GameAnalysis/Dishonored.md) | 아직 들키지 않은 상태 + 혼돈이라는 빚 | 단계적 판정 · 대가를 다음 미션으로 미루는 설계 |
| [Hitman: World of Assassination](./GameAnalysis/Hitman.md) | 여기 있어도 되는 사람이라는 상태 | 재료를 주고 플레이어가 사건을 만들게 하는 구조 |
| [BioShock Infinite](./GameAnalysis/BioShockInfinite.md) | 소금 (이 게임다운 해법의 잔량) | 분석 프레임의 중심축이 서사 쪽에 있던 사례 |
| [The Last of Us Part I](./GameAnalysis/TheLastOfUsPartI.md) | 물자와 소음, 두 개의 통화 | 자원을 제한 장치로 쓴 구조 · 큰 사이클과 작은 사이클 |
...
| [Stardew Valley](./GameAnalysis/StardewValley.md) | 하루 (시간과 에너지, 상한이 두 개) | 무엇을 포기할지 묻는 설계 · 최적화 욕구가 게임 밖으로 나가는 문제 |
| [세키로](./GameAnalysis/Sekiro.md) | 체간 — 양쪽이 서로 미는 주도권 | **경합하는 자원** · 습관을 교정하는 판정 설계 |
| [Death Stranding](./GameAnalysis/DeathStranding.md) | 적재량 (화물과 장비가 나눠 쓴다) | 이동을 핵심 액션으로 승격 · **해결이 지형에 남는 축적** |
| [Into the Breach](./GameAnalysis/IntoTheBreach.md) | 한 턴에 낼 수 있는 행동 (메카 3대) | 난이도를 문제 수 대 행동 수로 본 사례 · **정보를 공짜로 주는 설계** |
...
- [x] GameAnalysis 13개 문서를 10단계 템플릿으로 통일 (이후 Disco Elysium · Hades · 완다와 거상 · Devil May Cry 5 · Shadow Tactics · Katana ZERO · Monster Hunter: World · Sleeping Dogs · Dishonored · Hitman · BioShock Infinite · The Last of Us Part I · Resident Evil 2 · Alien: Isolation · DEATHLOOP · Metal Gear Solid V · Stardew Valley · 세키로 · Death Stranding · Into the Breach 추가로 33개)
...
- [x] 판매 중인 상품의 BM 분석 — 종료 전에 시세를 직접 수집해 확률·손익·기획 의도까지 추적
