...
> **원자료 출처**: 유튜브 채널 [`포피셜`](https://www.youtube.com/@PortiaOfficial) — 스펙업 효율표 시리즈.
> 아래 두 표의 **비용과 딜증 수치는 이 자료를 옮긴 것**이고,
> `비용당 효율`과 `격차`는 그 값을 나눠서 이 문서에서 도출한 것이다.
> 계산식은 각 표 아래에 그대로 적어 두었으므로 누구든 다시 확인할 수 있다.
...
계산은 `딜증% ÷ 비용`뿐이다. 위 표(방어구 100~103행, 무기 107~112행)의 값을 그대로 넣으면 나온다.

```
방어구   23강    1.15 ÷ 160.0 = 0.007188 %/G
        상재40   1.74 ÷  66.0 = 0.026364 %/G
        격차     0.026364 ÷ 0.007188 = 3.67배

무기     25강    1.40 ÷ 272.0 = 0.005147 %/G
        상재40   2.06 ÷  36.6 = 0.056284 %/G
        격차     0.056284 ÷ 0.005147 = 10.93배
```
...
> 이 격차는 원본 발표자료에 없었다. 같은 표를 나란히 놓고 나눠 보면서 뒤늦게 나온 값이다.
> **자료를 모으는 것과 자료를 대조하는 것은 다른 작업이라는 것이 이 한 줄에서 드러난다.** (8장 참고)
...
> **출처**: 유튜브 채널 [`포피셜`](https://www.youtube.com/@PortiaOfficial). 원본 발표자료(23p)에는 출처가 표기되어 있지 않아 이번에 보완했다.
...
- **인용 자료의 출처를 표기하지 않았다.** 4장의 비용·딜증 표와 5장의 회수 분석은 모두 외부 채널([포피셜](https://www.youtube.com/@PortiaOfficial))의 자료인데 발표자료에 출처가 없었다. 남의 데이터로 내 주장을 뒷받침할 때 출처가 없으면, 자료가 정확해도 검증할 수 없는 주장이 된다. **이 문서에서 뒤늦게 보완했고, 옮겨 온 수치와 이 문서에서 도출한 수치를 구분해 표기했다.**
