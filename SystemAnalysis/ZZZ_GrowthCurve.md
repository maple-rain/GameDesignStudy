...
> 우두머리 섬멸전(9,600)은 분류가 확정되지 않아 이 합계에서 제외했다.
> 재고와 유량 어느 쪽에도 넣지 않았으므로, 아래 모든 소요일은 **보수적인 상한값**이다. (7장 참고)
...
> 우두머리 섬멸전(9,600)은 분류 미확정이라 어느 쪽에도 넣지 않았다. (7장)

---

## 4. 결론

![신규 계정 로프꾼 레벨 성장 곡선](./images/zzz_growth_curve.svg)
...
| 목표 | 누적 필요 exp | 확정 재고(153,125) 기준 | 전체 재고(180,325) 기준 |
|---|---|---|---|
| 41레벨 | 153,820 | 0.2일 | **재고만으로 도달** |
| 50레벨 | 272,825 | 약 30일 | 약 23일 |
| 55레벨 | 580,455 | 약 108일 | 약 101일 |
| 60레벨 | 1,646,270 | **약 378일** | **약 371일** |
...
| 기준 | 재고로 도달하는 레벨 |
|---|---|
| 확정 재고만 | **40레벨** (41레벨까지 695 부족) |
| 이벤트·카고 트럭까지 전부 | 43레벨 |

**확정 재고 153,125는 41레벨 필요치 153,820에 695만큼 못 미친다.**
전체 재고의 0.45% 차이다. 이 정도로 붙어 있는 것은 우연으로 보기 어렵다.

즉 **이 게임은 상시 콘텐츠를 전부 소진하면 정확히 41레벨 문턱에 서도록 설계되어 있다.**
그 앞까지는 스토리와 콘텐츠로 빠르게 성장하다가, 이후로는 하루 3,950으로 고정된다.
접속 시간을 아무리 늘려도 이 값을 넘을 수 없다.

이벤트를 챙긴 유저만 43레벨까지 밀고 갈 수 있다.
...
| 우두머리 섬멸전 | 9,600 | 40분 (캐릭당 5분 × 8) | **240** | 주간 리셋 |
...
메인 스토리는 **최대 재고(68,925 — 확정 재고의 45%)인데 분당 효율은 최하위(31)다.**
...
- 우두머리 섬멸전을 유량으로 분류했으나, `캐릭당 1200 (600+600) × 8캐릭` 구조를 보면 1회성일 가능성이 있다.
  분류가 서지 않아 **일일 유량 합계에도 재고 총량에도 넣지 않았다.** 이 9,600을 어디에 넣느냐로 결과가 갈린다.
  주간 반복이면 일일 유량이 5,321로 올라 59→60레벨이 70일에서 약 52일로 줄고,
  1회성이면 확정 재고가 162,725가 되어 41레벨에 **도달한다** (42레벨 165,120에는 미달).
  **실측으로 확정해야 하는 첫 번째 항목이다.**
- 상시 이벤트와 카고 트럭은 **시점 의존 재고**로 분리했다. 시작 시점에 따라 접근 가능 여부가 달라지므로
  하나의 총량으로 다루면 결론이 흔들린다. 4장의 두 기준은 그래서 나눠 계산했다.
- 소요 시간 중 두 건은 원자료 표기가 **항목당인지 전체인지 불명**이라 분당 효율을 내지 않았다.
  상시 이벤트 `2시간 가량`, 전술 훈련 `15분 내외`. 해석에 따라 분당 효율이 10배까지 벌어진다.
- 카고 트럭과 로프꾼 전기는 경험치만 기록되어 있고 **소요 시간이 없다.**
- 파일명은 `데니분석`이지만 실제로는 경험치·레벨·시간 축이 완성되었고, **데니(재화) 축은 미완성**이다.

### 이번에 원자료를 다시 읽으며 정정한 것

| 항목 | 이전 | 정정 |
|---|---|---|
| 재고 총량 | 162,675 | **확정 153,125 / 전체 180,325** — 카고 트럭·로프꾼 전기·전술 훈련 3건이 빠져 있었다 |
| 제로 공동 220분 / 180분 | `추정` | **산출값** — `회당 10분 × 회차`로 원자료에서 그대로 계산된다 |
| 메인 스토리 | 효율 표에 없음 | **2,235분 / 분당 31** — 최대 재고인데 효율이 최하위 |
| 스토리 스킵의 효과 | 정성적 서술 | **최대 7시간** — 스킵 불가능한 의뢰가 시간의 81%를 차지한다 |

원자료에는 있었는데 문서로 옮기지 않은 값들이었다.
**분석이 끝난 뒤에도 원본을 다시 읽어야 하는 이유가 여기에 있다.**
...
- 우두머리 섬멸전의 반복 주기 확정 — 이 하나가 두 결론을 동시에 움직인다
- 카고 트럭 · 로프꾼 전기의 소요 시간 측정
- 상시 이벤트 `2시간 가량`, 전술 훈련 `15분 내외`의 단위 확정
